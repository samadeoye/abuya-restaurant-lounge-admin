...
class Product
{
    ...
    public static function getProducts($request)
    {
        ...
        {
            $params = [];
            if (isset($request['productTypeId']) && !empty($request['productTypeId']))
            {
                $params['productTypeId'] = "type={$request['productTypeId']}";
            }
            if (isset($request['productCategoryId']) && !empty($request['productCategoryId']))
            {
                $params['productCategoryId'] = "productCategoryId={$request['productCategoryId']}";
            }
    ...
    public static function getProductsList($request)
    {
        $rs = self::getProducts($request);
        if (count($rs) > 0)
        {
            $rows = [];
            $sn = 1;
            foreach ($rs as $r)
            {
                ...
                if (!empty($productCategories))
                {
                    ...
                    {
                        $arCat[] = $productCat['title'];
                    }
                    $productCategories = implode(', ', $arCat);
                }
...
EOQ;
                ...
                $row['edit'] = <<<EOQ
<button type="button" class="btn btn-primary btn-sm" onclick="editProduct('{$id}')">
    Edit
</button>
EOQ;
                $row['delete'] = <<<EOQ
...
EOQ;
    ...
    public static function addProduct($request)
    {
        ...
        $productCategoryIds = isset($request['product_categories']) ? $request['product_categories'] : [];
        ...
        $payload = [
            ...
            'price' => $price,
            'type' => $type,
            'menu_category' => 'main', //hardcoded for now. accepted options are: 'starter', 'main', 'dessert'
            'description' => $description,
        ];
        //product categories
        if (!empty($productCategoryIds))
        {
            $payload['product_category_ids[]'] = $productCategoryIds;
        }
    ...
    public static function updateProduct($request)
    {
        ...
        if ($id == 0 || empty($title) || $price == 0 || empty($type))
        {
            throw new Exception('Invalid request!');
        }
