...
$roomTitle = $description = $price = $featuredImage = '';
$shortDescription = '';
$arSelectedFacilities = [];

$title = 'Add New Room';
$formId = 'addUpdateRoomForm';
if ($action == 'updateroom')
{
    ...
    if ($rs)
    {
        ...
        $shortDescription = $rs['short_description'];
...
$rs = AbuyaAdmin\Facility\Facility::getFacilities();
$facilititesOptions = '';
if (count($rs) > 0)
{
    foreach ($rs as $r)
...
EOQ;
        ...
        $facilititesOptions .= <<<EOQ
<option value="{$r['id']}">{$icon} {$r['title']}</option>
EOQ;
    }
}
    ...
    <div class="modal-body">
        ...
        <div class="row">
            <div class="form-group col-md-12">
                <label>Title</label>
                <input type="text" class="form-control" name="title" id="title" value="<?php echo $roomTitle; ?>">
            </div>
        </div>
        ...
        <div class="row">
            <div class="form-group col-md-12">
                ...
                <input type="text" class="form-control" name="short_description" id="short_description" value="<?php echo $shortDescription; ?>">
            </div>
        </div>
...
$(document).ready(function() {
    ...
    $('#facilities').select2({
        ...
        dropdownParent: $('#facilities-wrapper'),
        width: '100%'
    });
    ...
    { ?>
        const selectedFacilities = <?php echo json_encode($selectedFacilities); ?>;
        $('#facilities').val(selectedFacilities).trigger('change');
        <?php
    }
    ?>
